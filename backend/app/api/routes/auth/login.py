...
async def request_login_otp(
    ...
    try:
        ...
        if user:
            ...
            if not await user_auth_service.verify_user_password(
                ...
                if remaining_attempts > 0:
                    error_message = (
                        f"Invalid creentails. You have {remaining_attempts}"
                        f"attempt{'s' if remaining_attempts !=1 else ''} remaing before"
                        "your account is temporarily locked."
                    )
                else:
                    error_message = (
                        "Invalid crendentials. Your account has been temporaily locked due "
                        f"to too many failed attempts. Please try again afger {settings.LOCKOUT_DURATION_MINUTES} minutes"
                    )
        ...
        return {
                "message": "if an account exists with this email, on OTP has been sent to it."
        }
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "action": "Plaease try again later"
            },
        )